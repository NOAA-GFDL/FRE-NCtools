...
void setup_bilinear_interp(int ntiles_in, const Grid_config *grid_in, int ntiles_out, const Grid_config *grid_out,
  ...
  if (ntiles_in != 6)
    mpp_error("Error from bilinear_interp: source mosaic should be cubic mosaic "
              "and have six tiles when using bilinear option");
  if (ntiles_out != 1)
    mpp_error("Error from bilinear_interp: destination mosaic should be "
  ...
  for (iter = 1; iter <= max_iter; iter++) {
    for (l = 0; l < ntiles_in; l++) {
      for (jc = 1; jc <= ny_in; jc++)
        for (ic = 1; ic <= nx_in; ic++) {
          /*------------------------------------------------------
          ...
          j_min = max(1, static_cast<int>(floor((grid_in[l].latt[n1] - dcub - latbegin) / dlat) - iter +
                                          1)); //TODO: possible error
          j_max = min(ny_out, static_cast<int>(ceil((grid_in[l].latt[n1] + dcub - latbegin) / dlat) + iter -
                                               1));//TODO: possible error
          ...
          for (j = j_min - 1; j < j_max; j++)
            for (i = i_min - 1; i < i_max; i++) {
              ...
              if (!found[n0]) {
                ...
                for (jcc = jc; jcc <= min(ny_in, jc + 1); jcc++)
                  for (icc = ic; icc <= min(nx_in, ic + 1); icc++) {
                    n1 = jcc * nxd + icc;
                    n2 = j * nx_out + i;
                    v1[0] = grid_in[l].xt[n1];
                    v1[1] = grid_in[l].yt[n1];
                ...
                /*------------------------------------------------
                        ...
                        ------------------------------------------------*/
                found[n0] = get_closest_index(&(grid_in[l]), grid_out, &(interp->index[3 * (j * nx_out + i)]),
                                              index[3 * l],
...
/*----------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------*/
void do_scalar_bilinear_interp(const Interp_config *interp, int vid, int ntiles_in, const Grid_config *grid_in,
                               const Grid_config *grid_out,
...
/*----------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------*/
void
do_vector_bilinear_interp(Interp_config *interp, int vid, int ntiles_in, const Grid_config *grid_in, int ntiles_out,
  ...
  for (n = 0; n < ntiles_in; n++)
    var_cubsph[n].data = (double *) malloc((nx_in + 2) * (ny_in + 2) * nz * sizeof(double));
    ...
    for (k = 0; k < nz; k++)
      for (j = 0; j < nyd; j++)
        for (i = 0; i < nxd; i++) {
          ...
          n2 = j * nxd + i;
          var_cubsph[n].data[n1] =
                  u_in[n].data[n1] * grid_in[n].vlon_t[3 * n2] + v_in[n].data[n1] * grid_in[n].vlat_t[3 * n2];
        }
  }
    ...
    for (k = 0; k < nz; k++)
      for (j = 0; j < nyd; j++)
        for (i = 0; i < nxd; i++) {
          ...
          n2 = j * nxd + i;
          var_cubsph[n].data[n1] =
                  u_in[n].data[n1] * grid_in[n].vlon_t[3 * n2 + 1] + v_in[n].data[n1] * grid_in[n].vlat_t[3 * n2 + 1];
        }
  }
    ...
    for (k = 0; k < nz; k++)
      for (j = 0; j < nyd; j++)
        for (i = 0; i < nxd; i++) {
          ...
          n2 = j * nxd + i;
          var_cubsph[n].data[n1] =
                  u_in[n].data[n1] * grid_in[n].vlon_t[3 * n2 + 2] + v_in[n].data[n1] * grid_in[n].vlat_t[3 * n2 + 2];
        }
  }
  ...
  for (k = 0; k < nz; k++)
    for (j = 0; j < ny_out; j++)
      for (i = 0; i < nx_out; i++) {
        ...
        n2 = j * nx_out + i;
        var_latlon[n1] = x_latlon[n1] * grid_out->vlon_t[3 * n2] + y_latlon[n1] * grid_out->vlon_t[3 * n2 + 1] +
                         z_latlon[n1] * grid_out->vlon_t[3 * n2 + 2];
      }
  ...
  for (k = 0; k < nz; k++)
    for (j = 0; j < ny_out; j++)
      for (i = 0; i < nx_out; i++) {
        n1 = k * nx_out * ny_out + j * nx_out + i;
        n2 = j * nx_out + i;
        var_latlon[n1] = x_latlon[n1] * grid_out->vlat_t[3 * n2] + y_latlon[n1] * grid_out->vlat_t[3 * n2 + 1] +
                         z_latlon[n1] * grid_out->vlat_t[3 * n2 + 2];
      }
        ...
        for (i = 0; i < nx_out; i++) {
          ...
          if (d_in[0] == missing || d_in[1] == missing || d_in[2] == missing || d_in[3] == missing) {
            ...
            data_out[k * nx_out * ny_out + n1] = d_in[0] * interp->weight[4 * n1] + d_in[1] * interp->weight[4 * n1 + 1]
                                                 + d_in[2] * interp->weight[4 * n1 + 2] +
                                                 d_in[3] * interp->weight[4 * n1 + 3];
          }
        }
  } else {
    for (k = 0; k < nz; k++)
...
void do_latlon_coarsening(const double *var_latlon, const double *ylat, int nlon, int nlat, int nz,
  ...
  switch (finer_steps) {
    case 0:
      if (nlon_crs != nlon || nlat_crs != nlat)
        mpp_error("bilinear_interp(do_latlon_coarsening): grid dimensions don't match");
    ...
    default:
      ...
      for (steps = 1; steps <= finer_steps; steps++) {
        ...
        redu2x(var_latlon_old, ylat_old, nlon_old, nlat_old, var_latlon_new, nlon_new, nlat_new, nz, has_missing,
               missvalue);
        free(var_latlon_old);
        free(ylat_old);
      }
      ...
      for (j = 2; j < nyfin - 2; j += 2) {
        ...
        for (i = 0; i < nxcrs; i++) {
          ...
          if (vartmp[n1 + j * nxcrs] == missvalue || vartmp[n1 + (j - 1) * nxcrs] == missvalue
            ...
            varcrs[n2 + j2 * nxcrs] = acosp[j] * (vartmp[n1 + j * nxcrs] +
                                                  0.5 * (vartmp[n1 + (j - 1) * nxcrs] + vartmp[n1 + (j + 1) * nxcrs]));
        }
      }
  } else {
    for (k = 0; k < nz; k++)
