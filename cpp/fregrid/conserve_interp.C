...
void setup_conserve_interp(int ntiles_in, const Grid_config *grid_in, int ntiles_out,
  ...
  if (opcode & READ) {
    for (n = 0; n < ntiles_out; n++) {
      if (interp[n].file_exist) { /* reading from file */
        ...
        if (opcode & CONSERVE_ORDER1)
          ...
          read_mosaic_xgrid_order2(interp[n].remap_file, i_in, j_in, i_out, j_out, xgrid_area, xgrid_clon,
                                   xgrid_clat);

        /*--- rescale the xgrid area */
        for (size_t i = 0; i < nxgrid; i++) xgrid_area[i] *= garea;
        ...
        for (size_t i = 0; i < interp[n].nxgrid; i++) {
    ...
    for (n = 0; n < ntiles_out; n++) {
      ...
      for (m = 0; m < ntiles_in; m++) {
        ...
        if (opcode & GREAT_CIRCLE) {
          ...
          jend = -1;
          for (j = 0; j <= ny_in; j++)
            for (size_t i = 0; i <= static_cast<size_t>(nx_in); i++) {
          ...
          if (opcode & CONSERVE_ORDER1) {
            ...
            if (g_nxgrid > 0) {
              ...
              for (size_t i = 0; i < static_cast<size_t>(g_nxgrid); i++) {
        ...
        if (nxgrid > 0) {
          ...
          if (nxgrid_prev == 0) {
            ...
            interp[n].area = (double *) malloc(interp[n].nxgrid * sizeof(double));
            interp[n].t_in = (int *) malloc(interp[n].nxgrid * sizeof(int));
            for (size_t i = 0; i < nxgrid_prev; i++) {
          ...
          for (size_t i = 0; i < (size_t) nx_in; i++) {
            ...
            if (cell_in[n].area[ii] > 0) {
              if (fabs(cell_in[n].area[ii] - grid_in[n].cell_area[ii]) / grid_in[n].cell_area[ii] <
                ...
                n2 = (j + 1) * (nx_in + 1) + i + 1;
                n3 = (j + 1) * (nx_in + 1) + i;
                x1_in[0] = grid_in[n].lonc[n0];
                y1_in[0] = grid_in[n].latc[n0];
                x1_in[1] = grid_in[n].lonc[n1];
                y1_in[1] = grid_in[n].latc[n1];
                x1_in[2] = grid_in[n].lonc[n2];
                y1_in[2] = grid_in[n].latc[n2];
                x1_in[3] = grid_in[n].lonc[n3];
                y1_in[3] = grid_in[n].latc[n3];
    ...
    if (opcode & WRITE) { /* write out remapping information */
      for (n = 0; n < ntiles_out; n++) {
        ...
        if (nxgrid > 0) {
          ...
          int fid, dim_ncells, dim_two, dims[4];
          ...
          for (size_t i = 0; i < static_cast<size_t>(nxgrid); i++) gdata_int[i]++;
          ...
          for (size_t i = 0; i < interp[n].nxgrid; i++) ldata_int[i] = interp[n].i_out[i] + grid_out[n].isc + 1;
          ...
          for (size_t i = 0; i < interp[n].nxgrid; i++) ldata_int[i] = interp[n].j_out[i] + grid_out[n].jsc + 1;
          mpp_gather_field_int(interp[n].nxgrid, ldata_int, gdata_int);
          mpp_put_var_value_block(fid, id_tile2_cell, start, nwrite, gdata_int);
    ...
    if (mpp_pe() == mpp_root_pe())
      printf("NOTE: done calculating index and weight for conservative interpolation\n");
  }

  /* check the input area match exchange grid area */
  if (opcode & CHECK_CONSERVE) {
    int nx1, ny1, max_i, max_j;
    ...
    for (n = 0; n < ntiles_out; n++) {
      for (size_t i = 0; i < (size_t) nx1 * ny1; i++) area2[i] = 0;
      for (size_t i = 0; i < interp[n].nxgrid; i++) {
...
void do_scalar_conserve_interp(Interp_config *interp, int varid, int ntiles_in, const Grid_config *grid_in,
  ...
  int nx1, ny1, nx2, ny2, i1, j1, i2, j2, tile, m, i, j, n1, n2;
  int k, n0;
  ...
  if (nz > 1 && cell_methods == CELL_METHODS_SUM)
    mpp_error("conserve_interp: cell_methods should not be sum when nz > 1");
  ...
  for (m = 0; m < ntiles_out; m++) {
    ...
    if (interp_method == CONSERVE_ORDER1) {
      if (has_missing) {
        for (size_t n = 0; n < interp[m].nxgrid; n++) {
          i2 = interp[m].i_out[n];
          j2 = interp[m].j_out[n];
          i1 = interp[m].i_in[n];
          j1 = interp[m].j_in[n];
            ...
            for (jj = j - 1; jj <= j + 1; jj++)
              for (ii = i - 1; ii <= i + 1; ii++) {
                ...
                if (field_in[n].data[n2] != missing) {
                  if (field_in[n].data[n2] > monotone_data[n].f_bar_max[n1])
                    monotone_data[n].f_bar_max[n1] = field_in[n].data[n2];
                  if (field_in[n].data[n2] < monotone_data[n].f_bar_min[n1])
                    monotone_data[n].f_bar_min[n1] = field_in[n].data[n2];
                }
              }
          }
      }
      ...
      for (size_t n = 0; n < interp[m].nxgrid; n++) {
        ...
        if (field_in[tile].data[n2] != missing) {
          if (field_in[tile].grad_mask[n1]) { /* use zero gradient */
            ...
            xdata[n] = field_in[tile].data[n2] + field_in[tile].grad_x[n1] * di +
                       field_in[tile].grad_y[n1] * dj;
          }
          if (monotonic) {
      ...
      for (size_t n = 0; n < interp[m].nxgrid; n++) {
        ...
        if (monotone_data[tile].f_max[n1] > monotone_data[tile].f_bar_max[n1]) {
          ...
          if (xdata[n] > monotone_data[tile].f_bar_max[n1]) {
            if (xdata[n] - monotone_data[tile].f_bar_max[n1] < TOLERANCE)
          ...
          /* z1l: Due to truncation error, we might get xdata[n] < f_bar_min[n1]. So
             we allow some tolerance. What is the suitable tolerance? */
          xdata[n] = f_bar + ((xdata[n] - f_bar) / (monotone_data[tile].f_min[n1] - f_bar)) *
                             (monotone_data[tile].f_bar_min[n1] - f_bar);
          if (xdata[n] < monotone_data[tile].f_bar_min[n1]) {
            if (monotone_data[tile].f_bar_min[n1] - xdata[n] < TOLERANCE)
    ...
    if (cell_methods == CELL_METHODS_SUM) {
      ...
      if ((target_grid)) {
        for (size_t n = 0; n < (size_t) nx2 * ny2; n++) out_area[n] = 0.0;
        for (size_t n = 0; n < interp[m].nxgrid; n++) {
          i2 = interp[m].i_out[n];
          j2 = interp[m].j_out[n];
          i1 = interp[m].i_in[n];
  ...
  if (opcode & CHECK_CONSERVE) {
    ...
    if (mpp_pe() == mpp_root_pe())
      printf("the flux(data*area) sum of %s: input = %g, output = %g, diff = %g. \n",
...
*******************************************************************************/
void
do_vector_conserve_interp(Interp_config *interp, int varid, int ntiles_in, const Grid_config *grid_in, int ntiles_out,
  ...
  int nx1, ny1, nx2, ny2, i1, j1, i2, j2, tile, m;
  ...
  for (int n = 0; n < ntiles_in; n++) {
    if (grid_in[n].rotate) {
      nx1 = grid_in[n].nx;
      ny1 = grid_in[n].ny;
      for (size_t i = 0; i < static_cast<size_t>(nx1 * ny1); i++) {
        tmp_x = u_in[n].data[i];
        tmp_y = v_in[n].data[i];
  ...
  for (m = 0; m < ntiles_out; m++) {
    ...
    for (size_t i = 0; i < static_cast<size_t>(nx2 * ny2); i++) {
      u_out[m].data[i] = 0.0;
      v_out[m].data[i] = 0.0;
    }
    for (size_t i = 0; i < static_cast<size_t>(nx2 * ny2); i++) out_area[i] = 0.0;
