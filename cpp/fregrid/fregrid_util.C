...
void
setup_boundary(const char *mosaic_file, int ntiles, Grid_config *grid, Bound_config *bound, int halo, int position);
...
void set_mosaic_data_file(int ntiles, const char *mosaic_file, const char *dir, File_config *file,
  ...
  if (dir) {
    if (strlen(dir) + strlen(str1) >= STRING) {
      mpp_error("set_mosaic_data_file(fregrid_util): length of str1 + "
  ...
  for (i = 0; i < ntiles; i++) {
    ...
    if (ntiles > 1) {
      ...
      if (strlen(str2) + strlen(tilename) > STRING - 5)
        mpp_error("set_mosaic_data_file(fregrid_util): length of str2 + "
...
void get_input_output_cell_area(int ntiles_in, Grid_config *grid_in, int ntiles_out, Grid_config *grid_out,
                                unsigned int opcode) {
...
void get_output_grid_from_mosaic(int ntiles, Grid_config *grid, const char *mosaic_file, unsigned int opcode,
  ...
  if (opcode & BILINEAR)
    mpp_error("fregrid_util: for bilinear interpolation, output grid can not got from mosaic file");
  ...
  *great_circle_algorithm = 0;
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    ny[n] = mpp_get_dimlen(g_fid, "ny");
    if (nx[n] % 2)
      mpp_error("fregrid_util(get_output_grid_from_mosaic): the size of dimension nx should be even (on supergrid)");
    if (ny[n] % 2)
      mpp_error("fregrid_util(get_output_grid_from_mosaic): the size of dimension ny should be even (on supergrid)");
    nx[n] /= 2;
    ny[n] /= 2;
    grid[n].nx = nx[n];
    grid[n].ny = ny[n];
    ...
    vid = mpp_get_varid(g_fid, "y");
    mpp_get_var_value(g_fid, vid, y);
    for (j = 0; j <= grid[n].nyc; j++)
      for (i = 0; i <= grid[n].nxc; i++) {
        jj = 2 * (j + grid[n].jsc);
        ii = 2 * (i + grid[n].isc);
...
*******************************************************************************/
void
get_output_grid_by_size(int ntiles, Grid_config *grid, double lonbegin, double lonend, double latbegin, double latend,
  ...
  if (finer_steps && !(opcode & BILINEAR))
    mpp_error("fregrid_util: finer_steps must be 0 when interp_method is not bilinear");
...
void do_vertical_interp(VGrid_config *vgrid_in, VGrid_config *vgrid_out, Grid_config *grid_out, Field_config *field,
                        int varid) {
      ...
      for (i = 0; i < ndim; i++) {
        ...
        switch (file_out[n].axis[i].bndtype) {
          case 1:
            ...
            if (file_out[n].axis[i].cart == 'X') {   /* x-axis */
              for (l = 0; l <= file_out[n].axis[i].size; l++)
                file_out[n].axis[i].bnddata[l] = grid_out[n].lonc1D[l] * R2D;
            } else if (file_out[n].axis[i].cart == 'Y') {
              for (l = 0; l <= file_out[n].axis[i].size; l++)
                file_out[n].axis[i].bnddata[l] = grid_out[n].latc1D[l] * R2D;
              ...
              for (l = 0; l <= file_out[n].axis[i].size; l++)
                file_out[n].axis[i].bnddata[l] = file_in[0].axis[i].bnddata[l];
            }
            break;
          case 2:
      ...
      if (mpp_pe() == mpp_root_pe()) {
        ...
        for (i = 0; i < ndim; i++) {
          if (file_out[n].axis[i].cart == 'T') {
            ...
            if ((file_out[n].axis[i].type == NC_INT || standard_dimension) && file_out[n].axis[i].cart == 'X')
              ...
              file_out[n].axis[i].dimid = mpp_def_dim(file_out[n].fid, file_out[n].axis[i].name,
                                                      file_out[n].axis[i].size);
        ...
        for (i = 0; i < ndim; i++) {
          if (file_out[n].axis[i].type == NC_INT && file_out[n].axis[i].cart == 'X') {
            ...
            if (file_out[n].axis[i].bndtype != 3)
              mpp_error("fregrid_util.c: axis bndtype must be 3 when axis vartype is NC_INT");
            ...
            if (file_out[n].axis[i].bndtype != 3)
              mpp_error("fregrid_util.c: x axis bndtype must be 3 when standard_dimension is true");
            /* define bounds variable */
            dims[0] = file_out[n].axis[i].dimid;
            dims[1] = dim_bnds;
            ...
            if (file_out[n].axis[i].bndtype != 3)
              mpp_error("fregrid_util.c: y axis bndtype must be 3 when standard_dimension is true");
            /* define bounds variable */
            dims[0] = file_out[n].axis[i].dimid;
            dims[1] = dim_bnds;
            ...
            file_out[n].axis[i].vid = mpp_def_var(file_out[n].fid, file_out[n].axis[i].name, file_out[n].axis[i].type,
                                                  1,
                                                  &(file_out[n].axis[i].dimid), 0);
            mpp_copy_var_att(file_in[0].fid, file_in[0].axis[i].vid, file_out[n].fid, file_out[n].axis[i].vid);
            if (file_out[n].axis[i].bndtype == 3)
              mpp_def_var_att(file_out[n].fid, file_out[n].axis[i].vid, "bounds", file_out[n].axis[i].bndname);

            switch (file_out[n].axis[i].bndtype) {
              case 3:
                dims[0] = file_out[n].axis[i].dimid;
                dims[1] = dim_bnds;
                file_out[n].axis[i].bndid = mpp_def_var(file_out[n].fid, file_out[n].axis[i].bndname,
                                                        file_out[n].axis[i].type, 2, dims, 0);
              ...
              case 1:
                ...
                if (j == ndim) { /* not found */
                  ...
                  file_out[n].axis[i].bndid = mpp_def_var(file_out[n].fid, file_out[n].axis[i].bndname,
                                                          file_out[n].axis[i].type, 1, dims, 0);
                  mpp_copy_var_att(file_in[0].fid, file_in[0].axis[i].bndid, file_out[n].fid,
                                   file_out[n].axis[i].bndid);
                }
                break;
              case 2:
                dims[0] = file_out[n].axis[i].dimid;
        ...
        for (l = 0; l < nscalar; l++) {
          ...
          for (i = 0; i < scalar_out[n].var[l].ndim; i++)
            dims[i] = file_out[n].axis[scalar_out[n].var[l].index[i]].dimid;
          ...
          for (i = 0; i < natts; i++) {
            ...
            if (!standard_dimension || !dst_is_latlon || strcmp(name, "coordinates"))
              ...
              save_coord = 1;
              for (unsigned int uj = 0; uj < ncoord_var; uj++) {
                found_var = 0;
                for (jj = 0; jj < scalar_in->nvar; jj++) {
                  if (!strcmp(coord_var[uj], scalar_in->var[jj].name)) {
                    found_var = 1;
                    break;
                  }
                }
...
void set_remap_file(int ntiles, const char *mosaic_file, const char *remap_file, Interp_config *interp,
  ...
  for (m = 0; m < ntiles; m++) {
    ...
    if (ntiles > 1) {
      start[0] = m;
      mpp_get_var_value_block(fid, vid, start, nread, tilename);
      if (strlen(str1) + strlen(tilename) > STRING - 5)
        mpp_error("set_output_remap_file(fregrid_util): length of str1 + "
                  "length of tilename should be no greater than STRING-5");
...
void get_input_data(int ntiles, Field_config *field, Grid_config *grid, Bound_config *bound,
  ...
  if (ndim != pos + 2)
    mpp_error("fregrid_util(get_input_data): mimstch between ndim and has_taxis/has_zaxis/has_naxis");
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    if (field[n].var[varid].cell_measures) {
      ...
      nread2[i] = ny;
      nread2[i + 1] = nx;
      mpp_get_var_value_block(field[n].var[varid].area_fid, field[n].var[varid].area_vid, start2, nread2,
                              field[n].area);
          ...
          for (k = 0; k < nz; k++)
            for (j = 0; j < ny; j++)
              for (i = 0; i < nx; i++) {
                ...
                jm1 = jj - 1;
                l = k * (nx + 2) * (ny + 2);
                if (field[n].data[l + jm1 * (nx + 2) + im1] == missing ||
                    field[n].data[l + jm1 * (nx + 2) + ii] == missing ||
                    field[n].data[l + jm1 * (nx + 2) + ip1] == missing ||
                    field[n].data[l + jj * (nx + 2) + im1] == missing ||
                    field[n].data[l + jj * (nx + 2) + ip1] == missing ||
                    field[n].data[l + jp1 * (nx + 2) + im1] == missing ||
                    field[n].data[l + jp1 * (nx + 2) + ii] == missing ||
                    field[n].data[l + jp1 * (nx + 2) + ip1] == missing)
...
void get_test_input_data(char *test_case, double test_param, int ntiles, Field_config *field,
  ...
  int halo, i, j, ii, n, nx, ny, l, nbound;
  double *data;
  Data_holder *dHold;
  char input_file[128];
  int fid, vid, dim[2];
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    if (!strcmp(test_case, "tanh_cosphi_costheta")) {
      for (j = 0; j < ny; j++)
        for (i = 0; i < nx; i++) {
          data[j * ny + i] = tanh(test_param * cos(grid[n].lont[(j + 1) * (nx + 2) + i + 1]) *
                                  cos(grid[n].latt[(j + 1) * (nx + 2) + i + 1]));
        }
    }
    if (!strcmp(test_case, "tanh_sinphi_sintheta")) {
      for (j = 0; j < ny; j++)
        for (i = 0; i < nx; i++) {
          data[j * ny + i] = tanh(test_param * sin(grid[n].lont[(j + 1) * (nx + 2) + i + 1]) *
                                  sin(grid[n].latt[(j + 1) * (nx + 2) + i + 1]));
        }
    } else if (!strcmp(test_case, "cosphi_costheta")) {
      for (j = 0; j < ny; j++)
        for (i = 0; i < nx; i++) {
          data[j * ny + i] =
                  cos(grid[n].lont[(j + 1) * (nx + 2) + i + 1]) * cos(grid[n].latt[(j + 1) * (nx + 2) + i + 1]);
        }
    } else if (!strcmp(test_case, "sinphi_costheta")) {
      for (j = 0; j < ny; j++)
        for (i = 0; i < nx; i++) {
          data[j * ny + i] =
                  sin(grid[n].lont[(j + 1) * (nx + 2) + i + 1]) * cos(grid[n].latt[(j + 1) * (nx + 2) + i + 1]);
        }
    } else
...
/*-------------------------------------------------------------------------
  ...
  -----------------------------------------------------------------------*/
void
write_field_data(int ntiles, Field_config *field, Grid_config *grid, int varid, int level_z, int level_n, int level_t) {
  double *gdata;
  double missing_value;
  short *data_i2;
  int *data_i4;
  int nx, ny, nz, n, ndim, i, data_size, pos;
  size_t *nwrite, *start;
  ...
  if (ndim != pos + 2)
    mpp_error("fregrid_util(write_field_data): mimstch between ndim and has_taxis/has_zaxis/has_naxis");
  ...
  for (n = 0; n < ntiles; n++) {
    ...
    switch (field[n].var[varid].type) {
      case NC_SHORT:
        ...
        if (mpp_pe() == mpp_root_pe())
          mpp_put_var_value_block(*(field[n].fid), field[n].var[varid].vid, start, nwrite, data_i2);
        free(data_i2);
        break;
      case NC_INT:
        ...
        if (mpp_pe() == mpp_root_pe())
          mpp_put_var_value_block(*(field[n].fid), field[n].var[varid].vid, start, nwrite, data_i4);
        free(data_i4);
        break;
      case NC_DOUBLE:
      case NC_FLOAT:
        if (mpp_pe() == mpp_root_pe())
          mpp_put_var_value_block(*(field[n].fid), field[n].var[varid].vid, start, nwrite, gdata);
        break;
      default:
    ...
    switch (bound->rotate[n]) {
      case ZERO:
        for (k = 0; k < nz; k++)
          for (j = js2; j <= je2; j++)
            for (i = is2; i <= ie2; i++)
              buffer[l++] = dHold[n].data[k * size2 + j * nx2 + i];
        break;
      case NINETY:
        for (k = 0; k < nz; k++)
          for (i = ie2; i >= is2; i--)
