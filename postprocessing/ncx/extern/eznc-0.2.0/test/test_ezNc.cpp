...
bool test_init(ez::ezTestRunner& runner) {
  ...
  std::vector<int> ids;
  ezassert( nc.init(buf, ids) == 0 );
  ids.push_back(3);
  ids.push_back(4);
  ezassert( nc.init(buf, ids) == 0 );

  std::vector<std::string> names;
  ezassert( nc.init(buf, names) == 0 );
  names.push_back("v7");
  names.push_back("v2");
  ezassert( nc.init(buf, names) == 0 );
