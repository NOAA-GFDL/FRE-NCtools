...
public:
  ...
  int copyVarDef(ezNc& fromnc, const char* name);
  // Copy using varid.
  int copyVarDef(ezNc & fromnc, int varid);
  // Copies all the var defs.
  int copyVarDefs(ezNc& fromnc);
  // Creates 64bit netcdf3 style file.
  int create64();
  int create64Clobber();
  // Creates HDF style file.
  int createHDF();
  int createHDFClobber();
  ...
  int createDim(const char* name, size_t size, bool isRecord=false);
  // Returns varid or -1 on error.
  int createVar(const char* name, nc_type type, int ndims, int* dimids, int storage=0, size_t* chunksizes=0, size_t size=0, size_t nelems=0, float preemption=0.0, int no_fill=1, void* fill_value=0, int shuffle=0, int deflate=0, int deflate_level=0, int checksum=0, int endian=NC_ENDIAN_NATIVE);
  // End definition mode.
  int enddef();
  // Cter.
  ezNc() { ncid = -1; clear(); }
  ...
  ~ezNc() { clear(); }
  // Get sorted dimids.
  void getAllDimIds(std::vector<int> &);
  // Get all names of dims sorted by dimids.
  void getAllDimNames(std::vector<std::string> &);
  // Gets list of names sorted as stored in file.
  void getAllGlobalAttNames(std::vector<std::string> &);
  // Return size in bytes for all vars, static and record.
  size_t getAllVarBytes();
  // Get sorted ids of vars (both static and record based).
  void getAllVarIds(std::vector<int> &);
  // Get all names of vars (both static and record based) sorted by varids.
  void getAllVarNames(std::vector<std::string> &);
  // Get var or global att value.
  int getAtt(const char* varname, const char* attname, ezNcBuffers& values);
  // Gets string contents for att, if exists. If not, returns empty string.
  int getAttString(const char* varname, const char* attname, std::string& string);
  // Get all var names that have the same name as var's dim names (CF convention). Use varid and get varids.
  void getCoordVars(int varid, std::vector<int>& varids);
  // Use varname and return var names.
  void getCoordVars(std::string& varname, std::vector<std::string>& varnames);
  // Get pointer from id by safely checking if id is valid. Returns 0 if invalid.
  inline ezNcDim* getDim(unsigned int id) { if (id >= dimsIdVector.size()) return 0; else return dimsIdVector[id]; }
  inline ezNcDim* getDim(const char* name) { if (name==0) return 0; return dimsNameMap[name]; }
  inline ezNcDim* getDim(std::string& name) { return dimsNameMap[name]; }
  // Get dimids used by all vars listed in varid vector.
  void getDimIds(std::set<int> & varids, std::set<int> & dimids);
  void getDimIds(std::vector<int> & varids, std::set<int> & dimids);
  // Get list of dimids in same order as names list.
  void getDimIds(std::vector<std::string> & dimnames, std::vector<int> & dimids);
  // Get dims names in proper order for a var.
  void getDimNames(int varid, std::vector<std::string> & dims);
  // Return vector of unique dim names used by each var listed.
  void getDimNamesForVars(std::vector<std::string> & vars, std::set<std::string> & dims);
  // Get dim sizes for a var, aka shape.
  void getDimSizes(int varid, std::vector<size_t> &);
  // Get dim sizes for a var, aka shape.
  void getDimSizes(std::string & name, std::vector<size_t> &);
  // Returns max number of dimensions used by list of var names.
  int getMaxNumDims(std::vector<std::string> &);
  // List of dims in the file.
  int getNumDims() { return dimsIdVector.size(); };
  // Get total number of variables.
  int getNumVars() { return getNumStaticVars() + getNumRecVars(); };
  // Get total number of static variables.
  int getNumStaticVars();
  // Get total number of record variables.
  int getNumRecVars();
  // Return size in bytes for all record vars.
  size_t getRecVarBytes();
  // Get pointer from id by safely checking if id is valid. Returns 0 if invalid.
  ...
  int putAtt(const char* varname, const char* attname, nc_type type, std::vector<std::string> & values, int first, int last);
  // More direct and efficient write of att data.
  int putAtt(const char* varname, const char* attname, nc_type type, ezNcBuffers& values);
  // Read portion of variable.
  int read(const char* name, ezNcBuffers*, size_t * start, size_t * count);
  int read(int varid, ezNcBuffers*, size_t * start, size_t * count);
  // Read entire variable.
  int read(int varid, ezNcBuffers*);
  // Returns human readable string for format type.
  static void typeToString(nc_type, std::string&);
  // Write portion of variable.
  int write(const char* name, ezNcBuffers*, size_t * start, size_t * count);
  int write(int varid, ezNcBuffers*, size_t * start, size_t * count);
};
//###########################################################################
...
int ezNc::copyDimDef(ezNc & fromnc, int dimid) {
  ...
  return createDim(fromdim->name.c_str(), fromdim->size, fromdim->isrec);
};
//###########################################################################
...
//###########################################################################
int ezNc::init(ezNcBuffers & buffers) {
  std::vector<int> varids;
  getAllVarIds(varids);
  return init(buffers, varids);
}
//###########################################################################
int ezNc::init(ezNcBuffers & buffers, const char* varname) {
  if (!varname) return 1;
  ezNcVar* var = varsNameMap[varname];
  if (!var) return 1;
  return init(buffers, var->varid);
}
//###########################################################################
int ezNc::init(ezNcBuffers & buffers, int varid) {
...
//###########################################################################
int ezNc::initEntire(ezNcBuffers & buffers, int varid) {
  std::vector<size_t> dimsizes;
  ezNcVar* var;
...
//###########################################################################
int ezNc::init(ezNcBuffers & buffers, std::vector<int> & varids) {
  int i=0;
  std::vector<size_t> dimsizes;
  int n = varids.size();
  ezNcVar* var;
...
//###########################################################################
int ezNc::init(ezNcBuffers & buffers, std::vector<std::string> & names) {
  std::vector<int> varids;
  int n = names.size();
  int i = 0;
  ezNcVar* var;
