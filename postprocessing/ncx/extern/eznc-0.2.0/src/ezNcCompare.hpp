...
public:
  ...
  void clear() { nc1.clear(); nc2.clear(); }
  // Compares size of dims for all dims in files. Both must have same dims.
  bool dimEquals();
  // Compares size of dims.
  bool dimEquals(std::string& dimname1, std::string& dimname2);
  // Compares all metadata and data.
  bool equals();
  // If formats equal.
  bool formatEquals() { return nc1.format == nc2.format; }
  // If all global atts equal.
  bool globalAttEquals();
  // If global atts equal.
  bool globalAttEquals(std::string& name1, std::string& name2);
  // Opens the files for comparison operations. Returns 0 if files opened ok.
  int init(std::string& filename1, std::string& filename2);
  // Compares formats, dims, global atts, var atts, var types, var shapes.
  bool metaDataEquals();
  // If all atts equal between two vars.
  bool varAttEquals(std::string& name1, std::string& name2);
  // If var atts equal.
  bool varAttEquals(std::string& vname1, std::string& aname1, std::string& vname2, std::string& aname2);
  ...
  bool varDataEquals(std::string& varname1, std::string& varname2);
  // If all vars meta data equal.
  bool varMetaDataEquals();
  // If metadata for two vars equal.
  bool varMetaDataEquals(std::string& varname1, std::string& varname2);
};
//######################################################################
...
bool ezNcCompare::varDataEquals(std::string& varname1, std::string& varname2) {
  ...
  ezNcBuffers buf1, buf2;
  nc1.init(buf1, var1->varid);
  nc2.init(buf2, var2->varid);
  buf1.allocate();
  buf2.allocate();
