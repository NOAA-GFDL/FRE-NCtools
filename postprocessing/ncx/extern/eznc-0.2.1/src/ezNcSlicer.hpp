/*
A file-global slice maker such that all dim slices are set once and then shared by all variables. 4 slice modes are possible: indexed min/max/stride, value min/max/stride, indices list, values list.
...
Index slices can be computed without an open nc object because they're independant of the file's data values.
...
sl.setDim...
sl.build(nc);
sl.getDim... // get details to build ezNcSlabIterator or do whatever you want.
...
public:
  ...
  inline T getDimSize(int id);
  inline T getDimSize(std::string& name);
  // Get list of indices for dim if it was index or value list sliced (slicetype==INDEXSLICE).
  inline void getDimIndices(int id, std::vector<T>& list);
  inline void getDimIndices(std::string&, std::vector<T>& list);
  // Get min/max/stride index slice info for dim (slicetype==RANGESLICE or STRIDESLICE).
  inline void getDimMinMaxStride(int id, T& mn, T& mx, T& stride);
  inline void getDimMinMaxStride(std::string&, T& mn, T& mx, T& stride);
  // Get list of dim sizes for a var.
  inline void getVarShape(int varid, std::vector<T>&);
  // Get list of dim sizes for a var.
  inline void getVarShape(std::string& varname, std::vector<T>&);
  // If is sliced convenience function.
  inline bool isDimSliced(std::string& name);
  // If is sliced convenience function.
  inline bool isDimSliced(int dimid);
  // If is sliced convenience function.
  inline bool isVarSliced(std::string& name);
  // If is sliced convenience function.
  inline bool isVarSliced(int varid);
  ...
  inline void setDimIndices(std::string& name, std::vector<std::string>& list, int begin, int end);
  ...
  inline void setDimValues(std::string& name, std::vector<std::string>& list, int begin, int end);
  // Set the min/max/stride index as strings.
  inline void setDimStride(std::string& name, std::string& mn, std::string& mx, std::string& stride);
  ...
  inline void setDimStrideValues(std::string& name, std::string& mn, std::string& mx, std::string& stride);
};
//#####################################################################
template <typename T>
