...
public:
  ...
  inline bool equals(ezNcBuffers& other, nc_type type=0);
  // Fill all arrays with a value.
  inline void fill(double, nc_type type=0);
  // Get length of array for type.
  inline size_t getSize(nc_type type);
  // Get values of given type array, and cast into output vector.
  template <typename T>
  inline void getValues(nc_type type, std::vector<T>& out);
  ...
  inline void find(nc_type type, std::vector<T>& list, std::vector<I>& indices);
  // Find index of first value greater-than-equal to a value in a buffer with same type as variant.
  inline int findFirstGreaterEqual(ezNcVariant&);
  inline int findFirstLessEqual(ezNcVariant&);
  inline int findLastGreaterEqual(ezNcVariant&);
  inline int findLastLessEqual(ezNcVariant&);
  // Check if an array's values are increasing. If all=false, only first 2 values are checked, otherwise all values are compared.
  inline bool increasing(nc_type, bool all=true);
  // Try to request raising ceiling size for type.
  inline void reserve(nc_type type, size_t size);
  ...
  template <typename T>
  inline void setMaxSizePerRec(nc_type nctype, std::vector<T>& dimlens, std::vector<int>& dimids, int recid=-1);
  // Set number of elements for a specific type.
  inline void setSize(nc_type type, size_t size);
...
//######################################################
size_t ezNcBuffers::getSize(nc_type type) {
  switch(type) { 
    case NC_CHAR: return nc;
    case NC_SHORT: return ns;
    case NC_INT: return ni;
    case NC_INT64: return nl;
    case NC_FLOAT: return nf;
    case NC_DOUBLE: return nd;
    case NC_UBYTE: case NC_BYTE: return nuc;
    case NC_USHORT: return nus;
    case NC_UINT: return nui;
    case NC_UINT64: return nul;
    default: return 0;
  }
}
//######################################################
template <typename T>
void ezNcBuffers::getValues(nc_type type, std::vector<T>& out) {
  #define PUSHBACK(P) {\
    out.reserve(n##P); \
    for(int j=0; j < n##P; ++j) out.push_back( (T) P[j] ); \
  }
  
  out.clear();
  switch(type) { 
    case NC_CHAR: PUSHBACK(c); break;
    case NC_SHORT: PUSHBACK(s); break;
    case NC_INT: PUSHBACK(i); break;
    case NC_INT64: PUSHBACK(l); break;
    case NC_FLOAT: PUSHBACK(f); break;
    case NC_DOUBLE: PUSHBACK(d); break;
    case NC_UBYTE: case NC_BYTE: PUSHBACK(uc); break;
    case NC_USHORT: PUSHBACK(us); break;
    case NC_UINT: PUSHBACK(ui); break;
    case NC_UINT64: PUSHBACK(ul); break;
  }
}
//###########################################################################
...
//###########################################################################
static void print(ezNcBuffers & b) {
  printf("ezNcBuffer\n");
...
void ezNcBuffers::reserve(nc_type type, size_t size) {
  #define SETMAX(S) if (size > S) S = size; 
  switch(type) { 
    case NC_CHAR: SETMAX(nc); break;
    case NC_SHORT: SETMAX(ns); break;
    case NC_INT: SETMAX(ni); break;
    case NC_INT64: SETMAX(nl); break;
    case NC_FLOAT: SETMAX(nf); break;
    case NC_DOUBLE: SETMAX(nd); break;
    case NC_UBYTE: case NC_BYTE: SETMAX(nuc); break;
    case NC_USHORT: SETMAX(nus); break;
    case NC_UINT: SETMAX(nui); break;
    case NC_UINT64: SETMAX(nul); break;
    default: break;
  }
};
//###########################################################################
