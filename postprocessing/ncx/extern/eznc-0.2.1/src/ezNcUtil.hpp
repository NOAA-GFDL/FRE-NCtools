...
static void DirName(std::string& path, std::string& prefix) {
...
static void formatToString(int f, std::string & s) {
  s.clear();
...
//###########################################################################
static int HandleNcStatus(int status) {
  if (status != NC_NOERR) {
    fprintf(stderr, "%s\n", nc_strerror(status));
  }
  return status;
...
template <typename T>
static bool isRevSeq(std::vector<T>& list) {
  int n = list.size();
  if (n == 1) {
    return true;
  }
...
// Check if list of numbers is a whole number increasing or decreasing sequence.
template <typename T>
static bool isSeq(std::vector<T>& list) {
  int n = list.size();
  if (n == 1) {
    return true;
  }
...
template <typename T>
static bool isStrideSeq(std::vector<T>& list) {
  int n = list.size();
  if (n <= 2) {
    return true;
  }
...
static long long osQueryPerformance() {
#ifdef WIN32
...
static int regex(std::string& pattern, std::vector<std::string>& strings, std::vector<std::string>& matches, bool extended=false, bool ignorecase=false) {
...
//##########################################################
// Rounds the fractional part of double to "dot-1" digits.
static double round_frac(double d, int dot) {
  double d1=1.0;
  while(--dot>=0) d1*=10.0;
  d*=d1;
  d=round(d);
...
template <typename T>
static void seq(std::vector<T>& list, int n) {
  list.clear();
  list.resize(n);
  for(int i=0; i < n; ++i)
    list[i] = i;
...
template <typename T>
static void seq(T* list, T mn, T mx) {
  if (list == 0) return;
  int n,i;
...
// Return simple sequence from min to max inclusive.  min can be greater than max for reversed sequence.
template <typename T>
static void seq(std::vector<T>& list, T mn, T mx) {
  list.clear();
  int n,i;
...
//###########################################################################
static int sizeOf(nc_type type) {
  switch(type) {
...
*/
static void MinMaxStrideToArray(std::vector<size_t> & mms, std::vector<size_t> & out, int offset=0, int max=0) {
  int mini=0;
  int maxi=max;
  int stride=1;
...
static void MinMaxStrideToArray(std::string & mms, std::vector<size_t> & out, int offset=0, int max=0) {
...
//###########################################################################
// Parses comma-delimited string.
static void MinMaxStrideToArray(const char* mms, std::vector<size_t> & out, int offset=0, int max=0) {
  std::stringstream ss(mms);
  std::vector<size_t> vec;
  size_t i;
  while (ss >> i) {
...
static nc_type StringToNcType(const char* str) {
...
//   output: int[2] = [-1,0]
template <typename T>
static void StringsToValues(std::vector<std::string> & vec, int first, int last, T* arr) {
  if (arr == 0) return;
  int ctr=0;
...
template <typename T>
static void StringsToValues(std::vector<std::string> & vec, int first, int last, std::vector<T>& arr) {
  arr.clear();
...
static void TmpFilename(std::string& out, const char* suffix) {
