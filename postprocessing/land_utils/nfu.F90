...
! ============================================================================
function nfu_put_vara_r8(ncid,name,start, nwrite, var) result(iret)
  integer :: iret ! return value
  integer     , intent(in) :: ncid   ! id of netcdf data set
  character(*), intent(in) :: name
  real(kind=8), intent(in) :: var(*)
  integer,      intent(in) :: start(:), nwrite(:)

  integer :: varid
  __NF_TRY__(nf_inq_varid(ncid, name, varid), iret, 7)
  iret = nf_put_vara_double(ncid,varid,start, nwrite, var)
7 return
end function
