...
# Get the location of the PKGLIBEXECDIR, set by automake, or use the
# user-defined location using the PKGLIBEXECDIR environment variable.
#my $pkg_libexec_dir = defined($ENV{PKGLIBEXECDIR}) ? $ENV{PKGLIBEXECDIR} : '@pkglibexecdir@';
my $pkg_libexec_dir = defined($ENV{PKGLIBEXECDIR}) ? $ENV{PKGLIBEXECDIR} : '/Users/seth.underwood/Development/NOAA-GFDL/FRE-NCtools/test.and.source.cleanup/build/src/';
...
# the build.
my $ncrcat = defined($ENV{NCRCAT}) ? $ENV{NCRCAT} : '@NCRCAT@';
$ncrcat .= " -t 2 --header_pad 65536";

my $ncks = defined($ENV{NCKS}) ? $ENV{NCKS} : '@NCKS@';
$ncks .= ' --header_pad 65536';

my $list_ncvars = defined($ENV{LIST_NCVARS}) ? $ENV{LIST_NCVARS} : "$pkg_libexec_dir/list_ncvars.csh";

my $ncdump = defined($ENV{NCDUMP}) ? $ENV{NCDUMP} : '@NCDUMP';

# Optional programs, if available
my $dmget = "dmget";
chmop ( $dmget = qx{which $dmget 2>&1} );
