#!/bin/csh -f
#
# $Id: split_ncvars_plevel.sh,v 1.1 2014/05/12 15:57:46 fms Exp $
# ------------------------------------------------------------------------------
...
#Balaji: add all variables that appear in a coordinate attribute
     set coords = ( `grep :coordinates .dump |cut -f2 -d\" |sed "s/ /\n/g" |sort -u` \
                    `grep "float geol[oa][nt]" .dump |awk '{print $2}' |cut -f1 -d\(` \
                    `egrep "float ps\(|float bk\(|float pk\(" .dump |awk '{print $2}' | cut -f1 -d\(` )
