...
unsigned long maxrss = 0; /* maximum memory used so far in kilobytes */
int print_mem_usage = 1;

static inline void check_mem_usage(void) {
  if (getrusage(RUSAGE_SELF, &ruse) == 0) {
    long rss = ruse.ru_maxrss;
    if (rss == 0) {
    ...
    fprintf(stderr,"current resident memory size=%lu KB, process max=%lu KB\n", rss, maxrss);
  }
}
