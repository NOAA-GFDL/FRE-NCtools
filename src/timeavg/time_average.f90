...
!
! This program averages variables stored in netCDF format over the time
! axis
!-----------------------------------------------------------------------
...
program time_average
   ...
   character(len=NF90_MAX_NAME) :: name, name_recdim, attname, name_time_bounds
...
1  continue
   ...
   ! create command_line string if not set by namelist
   if (len_trim(command_line) == 0) then
      command_line = 'use_end_time="'//ltoa(use_end_time)//'", '//&
         'verbose="'//ltoa(verbose)//'", '//&
         'add_cell_methods="'//ltoa(add_cell_methods)//'", '//&
         'skip_tavg_errors="'//ltoa(skip_tavg_errors)//'", '//&
         'suppress_warnings="'//ltoa(suppress_warnings)//'", '//&
         'frac_valid_data='//rtoa(frac_valid_data)//', '//&
         'user_deflation='//itoa(user_deflation)//', '//&
         'user_shuffle='//itoa(user_shuffle)//', '//&
         'file_name_out="'//trim(file_name_out)//'", '
      do ifile = 1, MAX_FILES
         if (file_names(ifile)(1:1) == ' ') exit
         command_line = trim(command_line)//', file_names('//itoa(ifile)//')="'//trim(file_names(ifile))//'"'
      enddo
   endif
   ...
   do ifile = 1, MAX_FILES
      if ( file_names(ifile)(1:1) == ' ' ) exit
      if (verbose) write (error_unit,*) 'Processing file ',trim(file_names(ifile))
...
33             format ('name=',a,2x,'skip=',l5,2x,'shape=',4i3)
      ...
      if (ifile == 1) then
         ...
         enddo
         ! add history and FRE-NCtools attributes
...
98                format ('WARNING: average information does not agree for variable ',a)
      ...
      if (istat /= NF90_NOERR) call error_handler (ncode=istat)
   enddo ! end of input file (ifile) loop
...
contains
   ...
   subroutine error_handler (string, ncode)
      ...
      if (present(ncode))  then
         write (error_unit, *) 'NETCDF ERROR in program time_average'
      else
         write (error_unit, *) 'ERROR in program time_average'
      endif
      if (present(string)) write (error_unit, *) trim(string)
      if (present(ncode))  then
         errstrg = NF90_STRERROR (ncode)
         write (error_unit, *) trim(errstrg)
      endif
      call abort ()
