  ...
  *********************************************************/
  ...
  if (strstr(file, ".nc")) {
    ...
    for (j = 0; j < nyp; j++)
      for (i = 0; i < nx; i++) {
        p1[0] = x[j * nxp + i] * D2R;
        p2[0] = x[j * nxp + i + 1] * D2R;
        p1[1] = y[j * nxp + i] * D2R;
        p2[1] = y[j * nxp + i + 1] * D2R;
        dx[j * nx + i] = great_circle_distance(p1, p2);
      }
    for (j = 0; j < ny; j++)
      for (i = 0; i < nxp; i++) {
        p1[0] = x[j * nxp + i] * D2R;
        p2[0] = x[(j + 1) * nxp + i] * D2R;
        p1[1] = y[j * nxp + i] * D2R;
        p2[1] = y[(j + 1) * nxp + i] * D2R;
    ...
    if (use_great_circle_algorithm) {
      ...
      for (j = 0; j < ny; j++)
        for (i = 0; i < nx; i++) {
          p1[0] = x[j * nxp + i] * D2R;
          p1[1] = x[(j + 1) * nxp + i] * D2R;
          p1[2] = x[(j + 1) * nxp + i + 1] * D2R;
          p1[3] = x[j * nxp + i + 1] * D2R;
          p2[0] = y[j * nxp + i] * D2R;
          p2[1] = y[(j + 1) * nxp + i] * D2R;
          p2[2] = y[(j + 1) * nxp + i + 1] * D2R;
          p2[3] = y[j * nxp + i + 1] * D2R;
      ...
      for (j = 0; j < ny; j++)
        for (i = 0; i < nx; i++) {
          p1[0] = x[j * nxp + i] * D2R;
          p1[1] = x[j * nxp + i + 1] * D2R;
          p1[2] = x[(j + 1) * nxp + i + 1] * D2R;
          p1[3] = x[(j + 1) * nxp + i] * D2R;
          p2[0] = y[j * nxp + i] * D2R;
          p2[1] = y[j * nxp + i + 1] * D2R;
          p2[2] = y[(j + 1) * nxp + i + 1] * D2R;
          p2[3] = y[(j + 1) * nxp + i] * D2R;
          area[j * nx + i] = poly_area(p1, p2, 4);
        }
    }
    /* currently set angle to 0 */
    for (j = 0; j < nyp; j++)
      for (i = 0; i < nxp; i++) angle_dx[j * nxp + i] = 0;
    free(xt);
    free(yt);
    free(xc);
    free(yc);
