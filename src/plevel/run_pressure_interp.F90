...
!***********************************************************************
! Copyright (C) 2011,2024 NOAA Geophysical Fluid Dynamics Lab,
! Princeton, NJ
!-----------------------------------------------------------------------

!-----------------------------------------------------------------------
! This program reads a NetCDF file and interpolates data from model
! levels to pressure levels. The input model grid is a hybrid
! sigma-pressure coordinate and the output pressure levels may be
! specified. The minimum required input fields are "bk", "pk", and "ps".
!
! This program also reads in a required namelist file, "plev.input.nml",
! which must have at least an "input" namelist, which will specify the
! input file name, the output file name, the output pressure levels,
! and the names of the fields to be interpolated.  The input and output
! files must be specified.  The namelist /input/ variables are:
!
!     character(len=256) :: in_file_name
!         Input NetCDF file.
!     character(len=256) :: out_file_name
!         Output NetCDF file.
!     character(len=NF90_MAX_NAME), dimension(MAX_FIELD_NAMES) :: field_names
!         Names of (additional) fields to be interpolated.  If
!         do_all_fields and do_all_3d_fields are false, then these
...
!     integer :: verbose = 1
!          Verbose level.  The higher the number, the more output.
!          Useful for debugging.
!     integer :: user_deflation = -1
!          Deflation level for new variables. -1 means use input file
!          settings.
!     integer :: user_shuffle = -1
!          Shuffle setting for new variables. -1 means use input file
!          settings.
!     character(len=4096) :: command_line
!          This program is typically called using the plevel.sh script.
!          This variable will hold the string used to launch this
!          program, which is added to the history attibute of the
!          written NetCDF file.  If this variable is empty, then the
!          namelist options with their values will be added to the
!          history attribte.
!
! A namelist "names" may be specified to change the expected variable
! names in the input file.  The namelist /names/ variables are:
!     character(len=NF90_MAX_NAME) :: temp_name = 'temp'
!     character(len=NF90_MAX_NAME) :: sphum_name = 'sphum'
!     character(len=NF90_MAX_NAME) :: hght_name = 'hght'
!     character(len=NF90_MAX_NAME) :: slp_name = 'slp'
!     character(len=NF90_MAX_NAME) :: ps_name = 'ps'
!     character(len=NF90_MAX_NAME) :: zsurf_name = 'zsurf'
!     character(len=NF90_MAX_NAME) :: bk_name = 'bk'
!     character(len=NF90_MAX_NAME) :: pk_name = 'pk'
!-----------------------------------------------------------------------
...
program run_pressure_interp
   ...
   use pinterp_utilities_mod, only:&
      & copy_axis,&
      & copy_variable,&
      & set_verbose_level,&
      & define_new_variable,&
   ...
   type field_type
      ...
      integer :: y_axis
      integer :: z_axis
      real, dimension(2) :: pack(2)
      real :: missval_in
      real :: unpkd_missval_in
      real :: missval_out
      real :: unpkd_missval_out
      logical :: static
      logical :: skip
      logical :: do_avg
      logical :: do_pack
   ...
   type axis_type
      ...
      character(len=NF90_MAX_NAME) :: name_out
      integer :: varid_in
      integer :: varid_out
      integer :: dimlen
...
1     close (10)
   ...
   if (len_trim(command_line) == 0) then
      command_line = 'in_file_name="'//trim(in_file_name)//'", '//&
           & 'out_file_name="'//trim(out_file_name)//'", '//&
           & 'do_all_fields='//ltoa(do_all_fields)//', '//&
           & 'do_all_3d_fields='//ltoa(do_all_3d_fields)//', '//&
           & 'mask_extrap='//ltoa(mask_extrap)//', '//&
           & 'time_beg='//itoa(time_beg)//', '//&
           & 'time_end='//itoa(time_end)//', '//&
           & 'time_inc='//itoa(time_inc)//', '//&
           & 'allow_zero_topog='//ltoa(allow_zero_topog)//', '//&
           & 'allow_zero_sphum='//ltoa(allow_zero_sphum)//', '//&
           & 'use_default_missing_value='//ltoa(use_default_missing_value)//', '//&
           & 'user_deflation='//itoa(user_deflation)//', '//&
           & 'user_shuffle='//itoa(user_shuffle)//','
      do i = 1, nlev_out
         command_line = trim(command_line)//' pout('//itoa(i)//')='//rtoa(pout(i))//','
      enddo
      do i = 1, num_field_names
         command_line = trim(command_line)//' field_names('//itoa(i)//')="'//trim(field_names(i))//'"'
         if (i < num_field_names) command_line = trim(command_line)//','
      enddo
   endif
   if (precision(ps) == precision(time)) then
      command_line = trim(command_line)//': precision=double'
   else
      command_line = trim(command_line)//': precision=float'
   endif
   ...
   if (verbose > 1) write (error_unit, *) 'num_field_names=',num_field_names
   do i = 1, num_field_names
      if (verbose > 1) write (error_unit, *) 'field_name=',trim(field_names(i))
   ...
   if (in_format == NF90_FORMAT_64BIT .or. in_format == NF90_FORMAT_CLASSIC) then
      if (user_deflation > 0) then
         write (*,*) 'Not using compression on NetCDF3 file'
         user_deflation = 0
      endif
   else
      if (verbose > 0) then
         if (user_deflation == -1) then
            write (*,*) 'Will use input file settings for NetCDF4 deflation level'
         else
            write (*,*) 'Will use NetCDF4 deflation level=', user_deflation
         endif
         if (user_shuffle == -1) then
            write (*,*) 'Will use input file settings for NetCDF4 shuffle'
         else
            write (*,*) 'Will use NetCDF4 shuffle=', user_shuffle
         endif
      endif
   endif
   ...
   if (verbose > 1) write (error_unit, *) 'Will use NetCDF4 deflation level=', new_var_deflation,&
   ...
   istat = NF90_CREATE (trim(out_file_name), cmode, ncid_out, chunksize=blksz)
   if (istat /= NF90_NOERR) call error_handler (ncode=istat)
   ...
   enddo
   ! Add to history and version attributes
   call nct_hst_att_cat(ncid_out, trim(command_line))
   ...
   if (istat == NF90_NOERR) then
      if (verbose > 1) write (error_unit, *) 'name_time_bounds=',trim(name_time_bounds)
   ...
   do i = 1, num_field_names
      ...
      write (error_unit, *) 'WARNING: unable to process field '//trim(field_names(i))
   enddo
   ...
   do itime = itime_beg, itime_end, itime_inc
      ...
      if ( verbose > 0 ) then
         if (itime == otime) then
            write (*,'(a,i8)') 'itime =', itime
         else
            write (*,'(a,2i8)') 'itime, otime =', itime, otime
         endif
      endif
      ...
      do i = 1, num_field_names
         ...
         if ( verbose > 1 ) then
            write (error_unit, *) 'Interp field = ',trim(Fields(i)%name)
         endif
...
contains
   ...
   subroutine interp_field_setup ( ncid_in, ncid_out, bk, pk,&
      ...
      if (do_hght) then
         if ( verbose > 1 ) write (error_unit, *) 'Computing field = hght'
         ...
         if ( verbose > 1 ) then
            write (error_unit, *) 'Interp field = ',trim(Fields(index_temp)%name)
            write (error_unit, *) 'Interp field = ',trim(Fields(index_sphum)%name)
         endif
         ...
         if ( verbose > 1 ) write (error_unit, *) 'Interp field = ',trim(Fields(index_temp)%name)
         ! interpolate temp
         Interp(1) = pres_interp_init ( pfull, pout, kbot,   &
            tin=temp_in,  tout=temp_out,  &
            use_extrap=.not.mask_extrap   )
      else if ((do_sphum).and.(.not.do_hght)) then
         if ( verbose > 1 ) write (error_unit, *) 'Interp field = ',trim(Fields(index_sphum)%name)
         ! interpolate sphum
         Interp(1) = pres_interp_init ( pfull, pout, kbot,   &
            qin=sphum_in, qout=sphum_out, &
            use_extrap=.not.mask_extrap   )
      ...
      ! temperature
      if (do_temp) then
         if ( verbose > 1 ) write (error_unit, *) 'Writing field = ',trim(Fields(index_temp)%name)
      ...
      ! specific humidity
      if (do_sphum) then
         if ( verbose > 1 ) write (error_unit, *) 'Writing field = ',trim(Fields(index_sphum)%name)
      ...
      ! height
      if (do_hght) then
         if ( verbose > 1 ) write (error_unit, *) 'Writing field = hght'
      ...
      ! sea level pressure
      if (do_slp) then
         if ( verbose > 1 ) write (error_unit, *) 'Computing field = slp'
         do j = 1, size(ps,2)
            do i = 1, size(ps,1)
               kb = kbot(i,j)
         ...
         if ( verbose > 1 ) write (error_unit, *) 'Writing field = slp'
   ...
   subroutine interp_field ( ncid_in, ncid_out, Interp, is, js, itime, otime, Field )
      ...
      select case ( Field%ndim )
       ...
       case default
         call error_handler ('ERROR: only 2-d or 3-d fields allowed')
      end select
   ...
   subroutine check_fields ( ncid )
      ...
      if (istat(1) /= NF90_NOERR) then
         write (error_unit,*) 'ERROR: required field does not exist: ',trim(names(1))
         ierr = 1
      endif
      if (istat(2) /= NF90_NOERR) then
         write (error_unit,*) 'WARNING: required field does not exist: ',trim(names(2))
         iwarn = 1
      endif
      if (istat(4) /= NF90_NOERR) then
         write (error_unit,*) 'ERROR: required field does not exist: ',trim(names(4))
         ierr = 1
      else
      ...
      if (do_hght .or. do_slp) then
         if (istat(3) /= NF90_NOERR) then
            if (allow_zero_topog) then
               write (error_unit,*) 'NOTE: field does not exist: ',trim(names(3)),' = 0 assumed'
            else
               write (error_unit,*) 'ERROR: required field does not exist: ',trim(names(3))
               ierr = 1
               do_hght = .false.
               do_slp  = .false.
            endif
         ...
         if (temp_present) then
            ...
            write (error_unit,*) 'ERROR: required field does not exist: ',trim(names(5))
            ierr = 1
            do_hght = .false.
            do_slp  = .false.
         endif
      ...
      if (do_hght) then
         if (sphum_present) then
            ...
            if (allow_zero_sphum) then
               write (error_unit,*) 'NOTE: field does not exist: ',trim(names(6)),' = 0 assumed'
            else
               write (error_unit,*) 'ERROR: required field does not exist: ',trim(names(6))
               ierr = 1
               do_hght = .false.
            endif
         endif
   ...
   subroutine get_input_variable_info ( name, ncid_in, Field )
      ...
      if (verbose > 2) write (error_unit,*) 'getting input variable info, name=',trim(name)
   ...
   subroutine copy_axis_information (ncid_in, name_in, ncid_out, decomp,&
      ...
      if (verbose > 2) write (error_unit,*) 'COPYING, name_in=',trim(name_in)
