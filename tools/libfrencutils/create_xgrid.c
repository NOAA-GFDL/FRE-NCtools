...
void get_lists(const double* lon_out, const double* lat_out, const int nx2,const int ny2,
               double* lon_min, double* lon_max, double* lon_avg,
               double* lat_min, double* lat_max,
               double* lon_list, double* lat_list,
               int* n2_list);
...
#define MAX_V 8
  int nx1, nx2, ny1, ny2, nx1p, nxgrid;
...
#pragma omp parallel
  ...
  get_lists(lon_out, lat_out, nx2, ny2, lon_out_min_list, lon_out_max_list,lon_out_avg,
            lat_out_min_list,lat_out_max_list, lon_out_list, lat_out_list, n2_list);
...
#pragma omp parallel
  ...
  get_lists(lon_in, lon_out, nx2, ny2, lon_out_min_list, lon_out_max_list,lon_out_avg,
            lat_out_min_list,lat_out_max_list, lon_out_list, lat_out_list, n2_list);
...
// helper routine to allocate lon/lat lists used in create_xgrid_2dx2d_order2
void get_lists(const double* lon_out, const double* lat_out, const int nx2,const int ny2,
               double* lon_min, double* lon_max, double* lon_avg,
               double* lat_min, double* lat_max,
               double* lon_list, double* lat_list,
  ...
  lon_min = (double *)malloc(nx2*ny2*sizeof(double));
  lon_max = (double *)malloc(nx2*ny2*sizeof(double));
  lat_min = (double *)malloc(nx2*ny2*sizeof(double));
  lat_max = (double *)malloc(nx2*ny2*sizeof(double));
  lon_avg = (double *)malloc(nx2*ny2*sizeof(double));
  n2_list     = (int *)malloc(nx2*ny2*sizeof(int));
  lon_list = (double *)malloc(MAX_V*nx2*ny2*sizeof(double));
  lat_list = (double *)malloc(MAX_V*nx2*ny2*sizeof(double));

#if defined(_OPENMP)
#pragma omp parallel for default(none) shared(nx2,ny2,lon_out,lat_out,lat_min, \
                                              lat_max,lon_min,lon_max, \
                                              lon_avg,n2_list,lon_list,lat_list)
#endif
  int ij;
  for(ij=0; ij<nx2*ny2; ij++){
    ...
    j2 = ij/nx2;
    n = j2*nx2+i2;
    n0 = j2*(nx2+1)+i2; n1 = j2*(nx2+1)+i2+1;
    n2 = (j2+1)*(nx2+1)+i2+1; n3 = (j2+1)*(nx2+1)+i2;
    ...
    lat_min[n] = minval_double(4, y2_in);
    lat_max[n] = maxval_double(4, y2_in);
    n2_in = fix_lon(x2_in, y2_in, 4, M_PI);
    if(n2_in > MAX_V) error_handler("create_xgrid.c: n2_in is greater than MAX_V");
    lon_min[n] = minval_double(n2_in, x2_in);
    lon_max[n] = maxval_double(n2_in, x2_in);
    lon_avg[n] = avgval_double(n2_in, x2_in);
    n2_list[n] = n2_in;
    for(l=0; l<n2_in; l++) {
      lon_list[n*MAX_V+l] = x2_in[l];
      lat_list[n*MAX_V+l] = y2_in[l];
    }
  }
}
