...
/*------------------------------------------------------------------------------
  ...
  grid is in radians.
  ----------------------------------------------------------------------------*/
double poly_area_dimensionless_acc(const double x[], const double y[], int n)
{
  double area = 0.0;
  int    i;
...
//TODO:
double poly_area2_acc(const double x[], const double y[], int n)
{
  double area = 0.0;
...
void rewindList_acc(void)
{
  ...
  curListPos = 0;
  if(!nodeList) nodeList = (struct Node_acc *)malloc(MAXNODELIST*sizeof(struct Node_acc));
  for(n=0; n<MAXNODELIST; n++) initNode_acc(nodeList+n);
...
struct Node_acc *getNext_acc()
{
  ...
  if(!nodeList) {
    nodeList = (struct Node_acc *)malloc(MAXNODELIST*sizeof(struct Node_acc));
    for(n=0; n<MAXNODELIST; n++) initNode_acc(nodeList+n);
  }
...
void addNode_acc_acc(struct Node_acc *list, struct Node_acc inNode_acc)
{

  addEnd(list, inNode_acc.x, inNode_acc.y, inNode_acc.z, inNode_acc.intersect, inNode_acc.u, inNode_acc.inbound, inNode_acc.isInside);
...
void printNode_acc_acc(struct Node_acc *list, char *str)
{
  ...
  if(list == NULL) error_handler("printNode_acc: list is NULL");
  if(str) printf("  %s \n", str);
  temp = list;
  while(temp) {
...
#ifndef __AIX
int inside_a_polygon_acc_(double *lon1, double *lat1, int *npts, double *lon2, double *lat2)
{

  int isinside;

  isinside = inside_a_polygon_acc(lon1, lat1, npts, lon2, lat2);
