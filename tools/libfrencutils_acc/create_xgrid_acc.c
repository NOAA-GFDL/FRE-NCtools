...
int get_nxgrid_upbound_2dx2d_order1_acc(const int input_grid_nlon, const int input_grid_nlat,
...
#pragma acc parallel loop independent reduction(+:nxgrid_upbound)
  for( int ij1=0 ; ij1<input_grid_ncells ; ij1++) {
    ...
    get_cell_vertices(ij1, input_grid_nlon, intput_grid_lon, input_grid_lat, x1, y1);

    input_cell_lat_min = minval_double(4, y1);
    input_cell_lat_max = maxval_double(4, y1);
    nvertices = fix_lon(x1, y1, 4, M_PI);
    input_cell_lon_min = minval_double(nvertices, x1);
    input_cell_lon_max = maxval_double(nvertices, x1);
    input_cell_lon_cent = avgval_double(nvertices, x1);
        ...
        for(ij=0; ij<nx2*ny2; ij++) {
          ...
          */
          ...
          if (  (n_out = clip_2dx2d_acc( x1_in, y1_in, n1_in, x2_in, y2_in, n2_in, x_out, y_out )) > 0) {
            double min_area;
            int    nn;
            xarea = poly_area_acc(x_out, y_out, n_out ) * mask_in[j1*nx1+i1];
  ...
  for(int j1=0; j1<ny1; j1++) for(int i1=0; i1<nx1; i1++) if( mask_in[j1*nx1+i1] > MASK_THRESH ) {
        ...
        for(j2=0; j2<ny2; j2++) for(i2=0; i2<nx2; i2++) {
            ...
            if (  (n_out = clip_2dx2d_great_circle_acc( x1_in, y1_in, z1_in, n1_in, x2_in, y2_in, z2_in, n2_in,
                                                    x_out, y_out, z_out)) > 0) {
              xarea = great_circle_area_acc( n_out, x_out, y_out, z_out ) * mask_in[j1*nx1+i1];
