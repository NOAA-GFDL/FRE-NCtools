...
contains
  ...
  subroutine horiz_interp_bilinear_new_1d(Interp, lon_in, lat_in, lon_out, lat_out, verbose, src_modulo)
    ...
    Interp%nlon_src = nlon_in
    Interp%nlat_src = nlat_in
    Interp%nlon_dst = nlon_out
    Interp%nlat_dst = nlat_out
    ...
    do n=1, nlat_out
      do m=1, nlon_out
        ...
        else  ! when the input grid is in not cyclic, the output grid should located inside the input grid
  ...
  !                                                                          *
  subroutine horiz_interp_bilinear_new_2d(Interp, lon_in, lat_in, lon_out, lat_out, verbose,&
      & src_modulo, new_search, no_crash_when_not_found)
    type(horiz_interp_type), intent(inout) :: Interp !< A derived-type variable containing indices
        !! and weights used for subsequent interpolations. To reinitialize this variable for a
        !! different grid-to-grid interpolation you must first use the "horiz_interp_del" interface.
    real, intent(in), dimension(:,:) :: lon_in !< Longitude (in radians) for source data grid.
    real, intent(in), dimension(:,:) :: lat_in !< Latitude (in radians) for source data grid.
    real, intent(in), dimension(:,:) :: lon_out !< Longitude (in radians) for source data grid.
    real, intent(in), dimension(:,:) :: lat_out !< Latitude (in radians) for source data grid.
    integer, intent(in), optional :: verbose !< flag for the amount of print output.
    logical, intent(in), optional :: src_modulo !< logical variable to indicate if the boundary
        !! condition along zonal boundary is cyclic or not. When true, the zonal boundary condition
        !! is cyclic.
    ...
    do n=1, nlat_out
      do m=1, nlon_out
        ...
        if (abs(a1+c1*y) < epsln) call mpp_error(FATAL, "horiz_interp_bilinear_mod: the denomenator is 0")
        ...
        Interp%wti(m,n,1)=1.0-x
        Interp%wti(m,n,2)=x
        Interp%wtj(m,n,1)=1.0-y
        Interp%wtj(m,n,2)=y
      end do
    end do
                ...
                if (lon .le. intersect(lat2,lon2,lat3,lon3,lat)) then ! east
                  if (lat .le. intersect(lon3,lat3,lon4,lat4,lon)) then ! north
                    if (lon .ge. intersect(lat4,lon4,lat1,lon1,lat)) then  ! west
                      found = .true.
                      Interp%i_lon (m,n,1) = i
                      Interp%i_lon (m,n,2) = ip1
                      Interp%j_lat (m,n,1) = j
                      Interp%j_lat (m,n,2) = jp1
                      exit J_LOOP
                    end if
                  end if
                end if
  ...
  !!
  !! \todo How to deal with truncation error?
  function inside_polygon(polyx, polyy, x, y)
    ...
    real, intent(in) :: x !< point to be tested
    real, intent(in) :: y !< point to be tested
        ...
        if (m==1 .and. n==1) then
          J_LOOP: do j=1, nlat_in-1
            do i=1, nlon_in
              ...
              polyx(1) = lon_in(i, j)
              polyy(1) = lat_in(i, j)
              polyx(2) = lon_in(ip1, j)
              polyy(2) = lat_in(ip1, j)
              polyx(3) = lon_in(ip1, jp1)
              polyy(3) = lat_in(ip1, jp1)
              polyx(4) = lon_in(i, jp1)
              polyy(4) = lat_in(i, jp1)
              if (lon .lt. lon_min .or. lon .gt. lon_max) then
                if (i .ne. nlon_in) then
                  cycle
                else
  ...
  subroutine horiz_interp_bilinear(Interp, data_in, data_out, verbose, mask_in,mask_out,&
    ...
    type(horiz_interp_type), intent(in) :: Interp  !< Derived-type variable containing interpolation
        !! indices and weights.  Returned by a previous call to horiz_interp_bilinear_new.
    ...
    integer, intent(in), optional :: verbose !< flag for the amount of print output.  verbose = 0,
        !! no output; = 1, min,max,means; = 2, still more
    real, intent(in), dimension(:,:), optional :: mask_in !< Input mask, must be the same size as
        !! the input data. The real value of mask_in must be in the range (0.,1.). Set mask_in=0.0
        !! for data points that should not be used or have missing data.
    real, intent(out), dimension(:,:), optional :: mask_out !< Output mask that specifies whether
        !! data was computed.
    real, intent(in), optional :: missing_value !<  Use the missing_value to indicate missing data.
    integer, intent(in), optional :: missing_permit !< numbers of points allowed to miss for the
        !! bilinear interpolation. The value should be between 0 and 3.
    logical, intent(in), optional :: new_handle_missing

    integer :: nlon_in, nlat_in, nlon_out, nlat_out, n, m
    integer :: is, ie, js, je, iverbose, max_missing, num_missing
    integer :: miss_in, miss_out, unit
    real :: dwtsum, wtsum, min_in, max_in, avg_in
    real :: min_out, max_out, avg_out, wtw, wte, wts, wtn
    ...
    nlon_in  = Interp%nlon_src
    nlat_in  = Interp%nlat_src
    nlon_out = Interp%nlon_dst
    nlat_out = Interp%nlat_dst
...
903    format ('          number of missing points = ',i6)
  ...
  subroutine horiz_interp_bilinear_del( Interp )
    type (horiz_interp_type), intent(inout) :: Interp !< A derived-type variable returned by
        !! previous call to horiz_interp_bilinear_new. The input variable must have allocated
        !! arrays. The returned variable will contain deallocated arrays.
  ...
  !!
  !! Indes of nearest data point to `value`.  If `value` is outside the domain of `array` then
  !! `indp = 1` or `ia` depending on whether `array(1)` or `array(ia)` is closest to `value`.
