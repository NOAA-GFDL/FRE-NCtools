...
# ------------------------------------------------
    gcp -v $compile_template $FTMPDIR/EXEC/
    gcp -v $postp_code_path $FTMPDIR/EXEC/postp/
    cd EXEC/postp/
    /home/fms/bin/mkmf -p cp_rvar -t $FTMPDIR/EXEC/$compile_template:t $postp_code_path:t
    gmake
    if ($status != 0) then
    ...
    gcp -v $rmvpr_code_path $FTMPDIR/EXEC/rmvpr/
    cd EXEC/rmvpr/
    /home/fms/bin/mkmf -p rmvp -t $FTMPDIR/EXEC/$compile_template:t $rmvpr_code_path:t
    gmake
    if ($status != 0) then
    ...
    gcp -v $lakes_code_path $FTMPDIR/EXEC/lakes/
    cd EXEC/lakes/
    /home/fms/bin/mkmf -p cplf -t $FTMPDIR/EXEC/$compile_template:t -c "-Duse_netCDF" \
    ...
    gcp -v $home_dir/exec/postp/cp_rvar $FTMPDIR/EXEC/postp/
    gcp -v $home_dir/exec/rmvpr/rmvp    $FTMPDIR/EXEC/rmvpr/
    gcp -v $home_dir/exec/lakes/cplf    $FTMPDIR/EXEC/lakes/
    chmod ugo+x $FTMPDIR/EXEC/postp/cp_rvar $FTMPDIR/EXEC/rmvpr/rmvp $FTMPDIR/EXEC/lakes/cplf
endif
...
$FTMPDIR/EXEC/postp/cp_rvar < fort.5
if ($status != 0) then
...
$FTMPDIR/EXEC/rmvpr/rmvp < fort.5
if ($status != 0) then
...
dmget $glcc_file
gcp -v gfdl:$glcc_file   gfdl:$FTMPDIR/DATA/
...
echo ""
echo ADD LAKES
$FTMPDIR/EXEC/lakes/cplf < fort.5
if ($status != 0) then
...
while ($k < $#river_input_files)
  @ k = $k + 1
  gcp OUTPUT/post_rmvp/river_network.tile"$k".nc $FTMPDIR/hydrography.tile"$k".nc
end
