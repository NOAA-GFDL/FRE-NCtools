                         ...
                         unsigned int *opcode, int save_weight_only)
{
  ...
  for(m=0; m<ntiles; m++) {
    ...
    if(ntiles > 1) {
      ...
      sprintf(xgrid[m].remap_file, "%s.%s.nc", str1, tilename);
    }
    else
      sprintf(xgrid[m].remap_file, "%s.nc", str1);
    /* check xgrid file to be read (=1) or write ( = 2) */
    if(!save_weight_only && mpp_file_exist(xgrid[m].remap_file)) {
      (*opcode) |= READ;
      xgrid[m].file_exist = 1;
    }
