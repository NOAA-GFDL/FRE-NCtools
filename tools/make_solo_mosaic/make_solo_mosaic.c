...
int main (int argc, char *argv[])
{
  ...
  for(n=0; n<ntiles; n++) {
    for(m=n; m<ntiles; m++) {
      ...
      count = get_align_contact(n+1, m+1, nxp[n], nyp[n], nxp[m], nyp[m], x[n], y[n], x[m], y[m],
				...
				jstart2, jend2);
      if(ncontact+count>MAXCONTACT) mpp_error("make_solo_mosaic: number of contacts is more than MAXCONTACT 1");
      for(l=0; l<count; l++) {
  ...
  for(n=0; n<ntiles; n++) {
    for(m=n+1; m<ntiles; m++) {
      ...
      int istart1[MAXCONTACT], iend1[MAXCONTACT], jstart1[MAXCONTACT], jend1[MAXCONTACT];
      int istart2[MAXCONTACT], iend2[MAXCONTACT], jstart2[MAXCONTACT], jend2[MAXCONTACT];
      count = get_overlap_contact(n+1, m+1, nxp[n], nyp[n], nxp[m], nyp[m], x[n], y[n], x[m], y[m],
				  istart1, iend1, jstart1, jend1, istart2, iend2, jstart2, jend2);
      if(ncontact+count>MAXCONTACT) mpp_error("make_solo_mosaic: number of contacts is more than MAXCONTACT 2");
      for(l=0; l<count; l++) {
	contact_tile1_istart[ncontact] = istart1[l];
	contact_tile1_iend  [ncontact] = iend1[l];
	contact_tile1_jstart[ncontact] = jstart1[l];
