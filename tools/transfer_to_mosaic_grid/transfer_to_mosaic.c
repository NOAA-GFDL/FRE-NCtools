  ...
  */
{
  ...
  int exist_ds_20_21_T, exist_ds_21_22_T;
  double unknown = 0.0; /* double unknown = -1.0E+10; */

  fid_old = mpp_open(old_file, MPP_READ);
  vid = mpp_get_varid(fid_old, "x_T");
  mpp_get_var_dimname(fid_old, vid, 1, xaxis_name);
  mpp_get_var_dimname(fid_old, vid, 0, yaxis_name);

  nx_T = mpp_get_dimlen(fid_old, xaxis_name);
  ny_T = mpp_get_dimlen(fid_old, yaxis_name);
...
/* Reading Ocean Grid Variables */
  ...
  if(mpp_var_exist(fid_old, "angle_T")) {
    ...
    for(i=0; i<ny_C * nx_C; i++) angle_T[i] = unknown;
  }
  if(mpp_var_exist(fid_old, "angle_E")) {
    vid = mpp_get_varid(fid_old, "angle_E"); mpp_get_var_value(fid_old, vid, angle_E);
  }
  else {
    for(i=0; i<ny_C * nx_C; i++) angle_E[i] = unknown;
  }
  if(mpp_var_exist(fid_old, "angle_N")) {
    vid = mpp_get_varid(fid_old, "angle_N"); mpp_get_var_value(fid_old, vid, angle_N);
  }
  else {
    for(i=0; i<ny_C * nx_C; i++) angle_N[i] = unknown;
  }

  if(mpp_var_exist(fid_old, "ds_00_10_T")) {
    vid = mpp_get_varid(fid_old, "ds_00_10_T"); mpp_get_var_value(fid_old, vid, ds_00_10_T);
  }
  else {
    for(i=0; i<nx_C; i++) ds_00_10_T[i] = unknown;
  }
  if(mpp_var_exist(fid_old, "ds_10_20_T")) {
    vid = mpp_get_varid(fid_old, "ds_10_20_T"); mpp_get_var_value(fid_old, vid, ds_10_20_T);
  }
  else {
...
*/
	...
	if(exist_ds_20_21_T) {
	  dy[(2*j  )*nxp + (2*i+2)] = ds_20_21_T[j*nx_T + i];
	}
	else {
	   if(j==0)
	...
	if (j==0) {
	   ...
	   dx[ (2*i+0)] = ds_00_10_T[i];
	   dx[ (2*i+1)] = ds_10_20_T[i];
  ...
  /* here cyclic boundary condition is assumed, just a quick fix. */
  for(j=1; j<nyp; j++) {
    x[j*nxp] = x[j*nxp+nx] - 360 ;
    y[j*nxp] = y[j*nxp+nx];
    angle_dx[j*nxp] = angle_dx[j*nxp+nx];
  }
  ...
  /* calculate cell area */
  {
    int    i;
    double *xrad, *yrad;
    xrad = (double *) malloc(nxp*nyp * sizeof(double));
    yrad = (double *) malloc(nxp*nyp* sizeof(double));
    for(i=0; i<nxp*nyp; i++) {
      xrad[i] = x[i]*PI/180.;
      yrad[i] = y[i]*PI/180.;
    }
    get_grid_area(&nx, &ny, xrad, yrad, area);
    free(xrad);
    free(yrad);
  }
  ...
  */
  ...
  /*-------ocean_hgrid.nc-------*/
  {
    ...
    size_t start[4], nwrite[4];
	  ...
	  mpp_put_var_value(fid, id_area, area);
          mpp_close(fid);
  }
}
/* ----------------------------------------------------------------]
...
{ 
  ...
  mpp_get_var_dimname(fid_old, vid, 1, xaxis_name);
  mpp_get_var_dimname(fid_old, vid, 0, yaxis_name);
  nx= mpp_get_dimlen(fid_old, xaxis_name );
  ny= mpp_get_dimlen(fid_old, yaxis_name );
